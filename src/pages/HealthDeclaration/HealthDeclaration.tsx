...
const HealthDeclaration: React.FC = () => {
  ...
  const language =
    localStorage.getItem(LocalStorageKey.LANGUAGE) === "1" || "" ? "vi" : "en";
  React.useEffect(() => {
    getData();
  ...
  const onSubmit = async (values: any) => {
    const questionsReq = await values.Form?.map(
      (item: IQuestionData, index: number) => {
        ...
        return {
          ...
          triggerId:
            index === 1 ? 1784 : index === 2 ? 1785 : index === 3 ? 1786 : null,
          type: item.type,
        };
      }
    );
  ...
  return (
    <Box>
      <Box className="title">
        <Typography component="h2" variant="h6" className="title">
          Health Declaration
        </Typography>
      </Box>
